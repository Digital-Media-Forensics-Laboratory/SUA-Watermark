...
class LinfPGDAttack(object):
    def __init__(self, model=None, device=None, epsilon=0.05, k=10, a=0.01, star_factor=0.3, attention_factor=0.3, att_factor=2, HiSD_factor=1, feat=None, args=None):
        ...
        self.k = k
        self.a = a
        # self.loss_fn = nn.functional.cosine_similarity.to(device)
    ...
    #         x_adv.clip(x-epsilon, x+epsilon)
    ...
    def IFGSM_perturb_attgan(self, X_nat, X_att, y, attgan, canny_num):
        """
        Vanilla Attack.
        """
        #iter_up = self.att_up
        ...
        for i in range(self.k):
            ...
            X = X + self.a * grad.sign()
            X.clip(min = X_nat-self.epsilon, max = X_nat+self.epsilon)
            X.clip(min = -1, max = 1)
        if self.up is None:
            eta = torch.mean(self.att_factor*(X - X_nat).detach_(), dim=0)
            self.up = eta
        else:
            eta = torch.mean(self.att_factor*(X - X_nat).detach_(), dim=0)
...
# *****************************************************************************************
    ...
    def universal_perturb_stargan_arcface(self, X_nat, y, c_trg, model, canny_num, arcface_model):
        """
        Vanilla Attack.
        """
        #iter_up = self.attention_up
        ...
        for i in range(self.k):
            ...
            if self.feat:
                output = feats[self.feat]
            output = arcface_model(output)
            model.zero_grad()
            arcface_model.zero_grad()
            # Minus in the loss means "towards" and plus means "away from"
            loss = self.loss_fn(output, y)
            # loss = -torch.cosine_similarity(output, y, dim = 1).mean()
            loss.backward()
            grad = X.grad
            X_adv = X + self.a * grad.sign()
            # print(X_adv.size())
    ...
    def universal_perturb_stargan_arcface_L2(self, X_nat, y, c_trg, model, canny_num, arcface_model):
        """
        Vanilla Attack.
        """
        #iter_up = self.attention_up
    ...
    def universal_perturb_expand(self, X_nat, gen, attack_source, expand_facor):
        ...
        for i in range(self.k):
            ...
            X_adv = X + self.a * expand_facor * (attack_source-self.up).sign()
            if self.up is None:
                eta = torch.mean(
                    torch.clamp(1 * (X_adv - X_nat), min=-self.epsilon, max=self.epsilon).detach_(),
                    dim=0)
                self.up = eta
            else:
                eta = torch.mean(
        ...
        #         X_adv = X + self.a * 0.35 * (attack_source-self.up).sign()
        #         if self.up is None:
        #             eta = torch.mean(
        #                 torch.clamp(1 * (X_adv - X_nat), min=-self.epsilon, max=self.epsilon).detach_(),
        #                 dim=0)
    ...
    def universal_perturb_expand_single_step(self, X_nat, gen, canny_num, attack_source, expand_facor):
        ...
        if self.up is None:
            ...
            eta = torch.mean(
                ...
                dim=0)
            self.up = self.up + eta
            # self.up =  torch.clamp(1 * (self.up + eta), min=-self.epsilon, max=self.epsilon).detach_()
        X = torch.clamp(X_nat + self.up, min=-1, max=1).detach_()
    ...
    def universal_perturb_expand_single_step_clip(self, X_nat, gen, canny_num, attack_source, expand_facor):
        ...
        if self.up is None:
            ...
            eta = torch.mean(
                ...
                dim=0)
            # self.up = torch.mean(
            #     torch.clamp(1 * (self.up + eta), min=-self.epsilon, max=self.epsilon),
            #     dim=0)
            self.up = 0.5*self.up + 0.5*eta
        X = torch.clamp(X_nat + self.up, min=-1, max=1).detach_()
    ...
    def universal_perturb_expand_single_step_weight(self, X_nat, gen, attack_source, expand_facor):
        ...
        if self.up is None:
            ...
            eta = torch.mean(
                torch.clamp(1 * (X_adv - X_nat), min=-self.epsilon, max=self.epsilon).detach_(),
                dim=0)
            self.up = torch.clamp(1 * (self.up + eta), min=-self.epsilon, max=self.epsilon)
            # self.up = 0.5*self.up + 0.5*eta
        X = torch.clamp(X_nat + self.up, min=-1, max=1).detach_()
