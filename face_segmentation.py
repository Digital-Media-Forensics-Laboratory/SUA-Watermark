...
def get_seg_face(img_path):
    ...
    # cv2.imwrite("KOBE1.png", image_bgra)

    image_bgr = merge_add_mask(image, mask)
    cv2.imwrite("KOBE2.png", image_bgr)

def celeba_clean_face(img_path):
    cleannum  = 0
    ...
    for img in img_path_list:
        ...
        landmarks = get_landmarks(image)
        if len(landmarks) != 0:
            with open("./img_name_cleaned.txt",'a') as f:
...
def get_seg_face_image(img):
    ...
    return image_bgr,flag

if __name__ == "__main__":
    get_seg_face('demo_input.png')
