...
class Solver(object):
    ...
    def test_universal_model_level_attack_arcface(self, i, x_real, c_org, pgd_attack, canny_num, arcface_model):
        """Universal Attack by Huang Hao"""
        ...
        for idx, c_trg in enumerate(c_trg_list):
            ...
            with torch.no_grad():
                ...
                # x_real_mod = self.blur_tensor(x_real_mod) # use blur
                gen_noattack, gen_noattack_feats = self.G(x_real_mod, c_trg)
                features_noattack = arcface_model(gen_noattack)

            # Attacks
            x_adv, perturb = pgd_attack.universal_perturb_stargan_arcface(x_real, features_noattack, c_trg, self.G, canny_num, arcface_model)  # Vanilla
