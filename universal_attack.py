...
if opt.backbone == 'resnet18':
    ...
    arcface_model = resnet34()
elif opt.backbone == 'resnet50':
    arcface_model = resnet_face50(opt.use_se)
arcface_model = DataParallel(model)
arcface_model.load_state_dict(torch.load(opt.test_model_path))
...
for i in range(1):
    for idx, (img_a, att_a, c_org) in enumerate(tqdm(attack_dataloader)):
        ...
        canny_num = [1,1,1,1,1,1,1,1]
        canny_num = torch.tensor(canny_num)
        canny_num = torch.unsqueeze(canny_num,1)
        canny_num = canny_num.expand(8,3)
        canny_num = torch.unsqueeze(canny_num,2)
        canny_num = canny_num.expand(8,3,256)
        canny_num = torch.unsqueeze(canny_num,3)
        canny_num = canny_num.expand(8,3,256,256)
        canny_num = canny_num.cuda() if args_attack.global_settings.gpu else canny_num
        canny_num = canny_num.float()
        
        # print(canny_num.size())
        # print(canny_num)

        # attack stargan
        solver.test_universal_model_level_attack(idx, img_a, c_org, pgd_attack, canny_num)

        # attack attentiongan
        attentiongan_solver.test_universal_model_level_attack(idx, img_a, c_org, pgd_attack, canny_num)
        ...
        with torch.no_grad():
           ...
           mask[mask>0.5] = 1
           mask[mask<0.5] = 0
        pgd_attack.universal_perturb_HiSD(img_a.cuda(), transform, F, T, G, E, reference, x_trg+0.002, gen_models, mask, canny_num)
        ...
        for i, att_b in enumerate(att_b_list):
            ...
            x_adv, perturb = pgd_attack.universal_perturb_attgan(img_a, att_b_, gen_noattack, attgan, canny_num)
